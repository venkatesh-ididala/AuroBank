...
export const createBankAccount = async ({
  ...
  try {
    if (!userId || !bankId || !accessToken) {
      throw new Error("Missing required parameters to create bank account");
    }
    const { database } = await createAdminClient();
...
export const exchangePublicToken = async ({
  ...
  try {
    ...
    if (!accessToken || !itemId) {
      throw new Error("Failed to retrieve access token or item ID");
    }
